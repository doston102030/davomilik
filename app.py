...
let toastTimer=null;function showSelectionToast(message){const toast=document.getElementById('selectionToast');document.getElementById('selectionToastText').textContent=message;toast.classList.add('show');window.clearTimeout(toastTimer);toastTimer=window.setTimeout(()=>toast.classList.remove('show'),3200)}
...
csvInput.addEventListener('change',event=>update(event.currentTarget));if(gnpInput)gnpInput.addEventListener('change',event=>update(event.currentTarget));document.getElementById('csvPick')?.addEventListener('click',()=>csvInput.click());document.getElementById('gnpPick')?.addEventListener('click',()=>gnpInput.click());
